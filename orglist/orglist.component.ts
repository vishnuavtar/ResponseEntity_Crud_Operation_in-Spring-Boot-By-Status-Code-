...
import { OrgDialogComponent, OrgService } from 'app/org'; 
//from './org.service';
import { DataCommunicationService } from 'app/shared';
import { MatDialogComponent } from 'app/shared/components';
import { saveAs } from 'file-saver';
import { Org } from 'app/org/models';
//import { OrglistServiceService } from './orglist-service.service';
import { data } from 'jquery';

@Component({
 selector: 'jhi-orglist',
...
export class OrglistComponent implements OnInit, OnDestroy {
  ...
  sveCat = 'Active';
  productList:string[] = ['DECYFIR', 'DETCT']
  selectedProduct = 'DECYFIR'
  ...
  constructor(
    ...
    //private orglistService:OrglistServiceService
  ) {
    this.pagerInfo = {
      currentPage: 1,
      pageSize: 20,
 ...
 // this.orglistService. getOrgList().subscribe(data => this.orgList = data);
  }
  ...
  getAllOrgs(pager?: any, orgstatus?: any) {
    const req: any = {
      ...
      sort: ['name,ASC'],
      status: orgstatus,
      decyfir: this.selectedProduct == 'DECYFIR' ? true : false
  ...
  getListByProduct(productName:string) {
